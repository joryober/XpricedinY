...
function App() {
  ...
  const handleChangeXAmount = (event) => {
    setAmounts({
      ...
      y: (event.target.value * coinXPrice) / coinYPrice,
    });
  };

  const handleChangeYAmount = (event) => {
    setAmounts({
      x: (event.target.value * coinYPrice) / coinXPrice,
  ...
  const handleClick = () => {
    ...
    fetch(
      ...
      .then((response) => {
        ...
        setAmounts({
          x: 1,
          y: response[coinX]["usd"] / response[coinY]["usd"],
        });
        setLoading(false);
      })
  ...
  return (
    <>
      ...
      <div className="flex py-5 items-center justify-around">
        <p>
          I want <input type="text" value={coinX} onChange={handleChangeX} />{" "}
          priced in <input type="text" value={coinY} onChange={handleChangeY} />
        </p>
      ...
      {coinXPrice && coinYPrice && (
        <h3>
          <input
            ...
            type="text"
            value={amounts.x}
            onChange={handleChangeXAmount}
          />
          {` ${displayCoinX} is equal to `}
          <input
            className="h-8"
            type="text"
            value={amounts.y}
            onChange={handleChangeYAmount}
          />
          {` ${displayCoinY}`}
        </h3>
      )}
      <footer className="h-8 fixed bottom-0 left-0 z-20 w-full p-4 bg-white border-t border-gray-200 shadow md:flex md:items-center md:justify-between md:p-6 dark:bg-gray-800 dark:border-gray-600 text-gray-500">
